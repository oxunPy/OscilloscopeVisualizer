...
public class LoginService extends BaseService {
    ...
    public UserProperty getLoggedUser(String login) {
        ...
        try {
            ...
            UserEntity entity = userDao.get(hql, new Object[]{login});
    ...
    public boolean authenticatePassword(String login, String password) {
        ...
        try {
            UserEntity entity = userDao.get("from DealerUser where login=? and pass=?", new Object[]{login, encryptedPassword});
            result = entity != null;
        } catch (Exception ex) {
            log.print(StringConfig.getValue("err.db.login.password") + "\n" + ex);
        }
    ...
    public UserProperty userProperty(Serializable id) {
        ...
        UserProperty userProperty = UserProperty.newInstance(userDao.get(UserEntity.class, id), true, true);
